...
namespace bdsp
{
	PresetMenu::PresetMenu(PresetManager* p)
		:favColumnHeader(p->holder->universals, true),
		vp(this),
		clearSearch(p->holder->universals),
		GUI_Universals::Listener(p->holder->universals),
		search(p->holder->universals)
	{
		parentManager = p;
		...
		Shape X(createX(juce::Rectangle<float>(0, 0, 1, 1), 0.15, true), p->pc.presetText, juce::Rectangle<float>(0.2, 0.2, 0.6, 0.6));
		...
		static const unsigned char pathData[] = StarData;
		juce::Path path;

		path.loadPathFromData(pathData, sizeof(pathData));


		Shape s(path, parentManager->pc.tagBKGDSelected, juce::Rectangle<float>(0, 0, 1, 1));
		...
		{
			columnHeaders.add(new TextButton(parentManager->holder));
		...
		Popup = std::make_unique<PopupMenu>(this, p->holder);
		Popup->List.onSelect = [=](int i)
		{
			...
			case PresetItemActions::Override:

				p->overridePreset(highlightQueue.getLast());
				break;
		...
		Popup->setColors(BDSP_COLOR_LIGHT, BDSP_COLOR_DARK, parentManager->pc.presetText);
	...
	{
		auto menuBounds = getLocalBounds().getProportion(juce::Rectangle<float>(0, 0.4, 1, 0.6));

		auto searchRect = juce::Rectangle<int>(0, 0, getWidth(), menuBounds.getY()).getProportion(juce::Rectangle<float>(0.05, 0.1, 0.9, 0.1));
		search.setBounds(searchRect.withTrimmedRight(clearSearch.isVisible() ? searchRect.getHeight() : 0));
		search.applyFontToAllText(parentManager->holder->universals->Fonts[parentManager->titleBar.getFontIndex()].font.withHeight(search.getHeight() * 0.9));
		search.setBorder(juce::BorderSize<int>(parentManager->holder->universals->roundedRectangleCurve / 4));

		clearSearch.setBounds(searchRect.withWidth(search.getHeight()).withRightX(searchRect.getRight()));

		tags->calculateNewSize(juce::Rectangle<int>(0, 0, getWidth(), menuBounds.getY()).getProportion(juce::Rectangle<float>(0, 0.3, 1, 0.6)));
	...
	void PresetMenu::GUI_UniversalsChanged()
	{
		...
		search.setColour(juce::TextEditor::textColourId, parentManager->pc.universals->colors.getColor(parentManager->pc.presetText));
		search.setColour(juce::TextEditor::highlightedTextColourId, parentManager->pc.universals->colors.getColor(parentManager->pc.presetText));
		search.setColour(juce::TextEditor::highlightColourId, parentManager->pc.universals->colors.getColor(parentManager->pc.presetText).withAlpha(parentManager->holder->universals->lowOpacity));
		search.setColour(juce::CaretComponent::caretColourId, parentManager->pc.universals->colors.getColor(parentManager->pc.browserBKGD));
		search.setInputRestrictions(juce::jmax(BDSP_MAX_PRESET_NAME_LENGTH, BDSP_MAX_TAG_NAME_LENGTH));

		search.setLookAndFeel(&parentManager->LNF);
		search.setTextToShowWhenEmpty("Search", parentManager->pc.universals->colors.getColor(parentManager->pc.browserBKGD).interpolatedWith(parentManager->pc.universals->colors.getColor(parentManager->pc.presetText), 0.5));
		search.setJustification(juce::Justification::centred);


		//Popup->setColour(juce::Label::backgroundColourId, parentManager->pc.universals->colors.getColor(BDSP_COLOR_LIGHT));
		//Popup->setColour(juce::Label::backgroundWhenEditingColourId, parentManager->pc.universals->colors.getColor(BDSP_COLOR_DARK));
		//Popup->setColour(juce::Label::outlineColourId, parentManager->pc.universals->colors.getColor(parentManager->pc.outline));
		//Popup->setColour(juce::Label::textColourId, parentManager->pc.universals->colors.getColor(parentManager->pc.presetText));
	}

	void PresetMenu::componentMovedOrResized(juce::Component& component, bool wasMoved, bool wasResized)
	{
		auto itemH = getHeight() * itemHRatio;
		auto itemB = itemH * 0.25;

		vp.setBounds(getLocalBounds().withTop(tags->getBottom() + itemH + 2 * itemB));

		vp.setScrollBarThickness(parentManager->scrollBarW);

		vp.setScrollDistance(itemH + itemB);
		...
		for (; i < items.size(); i++) // set bounds of all visible items
		{
			...
			items[i]->setBounds(0, i * itemH + (i + 1) * itemB, vp.getWidth() - 1.5 * vp.getScrollBarThickness(), itemH);
		}
		...
		favColumnHeader.setBounds(juce::Rectangle<int>().leftTopRightBottom(0, vp.getY() - itemH, splits[0], vp.getY()).withTrimmedBottom(itemB / 2));
	...
	{
		g.setColour(parentManager->pc.universals->colors.getColor(parentManager->pc.searchBarBKGD));
		g.fillRoundedRectangle(juce::Rectangle<int>(search.getPosition(), clearSearch.getBounds().getBottomRight()).toFloat().expanded(parentManager->holder->universals->roundedRectangleCurve / 2), parentManager->holder->universals->roundedRectangleCurve);

		drawDivider(g, juce::Line<float>(vp.getX(), vp.getY(), vp.getRight() - 1.5 * vp.getScrollBarThickness(), vp.getY()), parentManager->pc.universals->colors.getColor(parentManager->LNF.pc->searchBarBKGD), parentManager->holder->universals->dividerSize);
	...
	{
		//g.setColour(parentManager->pc.universals->colors.getColor(parentManager->pc.searchBarBKGD));
		//g.drawRoundedRectangle(juce::Rectangle<int>(search.getPosition(), clearSearch.getBounds().getBottomRight()).toFloat(), parentManager->holder->universals->roundedRectangleCurve, parentManager->holder->universals->roundedRectangleCurve / 5);

		//================================================================================================================================================================================================


		//g.setColour(parentManager->pc.universals->colors.getColor(parentManager->LNF.pc->presetText));
		//g.drawHorizontalLine(vp.getY(), parentManager->holder->universals->roundedRectangleCurve, getWidth() - parentManager->holder->universals->roundedRectangleCurve);
		...
		g.setFont(parentManager->LNF.universals->Fonts[1].font.withHeight(favColumnHeader.getHeight() * 0.9));
		for (int i = 0; i < columnHeaders.size(); i++)
		{
			/*	ga.clear();
			...
			{
				g.setColour(parentManager->pc.universals->colors.getColor(parentManager->LNF.pc->sortedColumnText));
	...
	void PresetMenu::popupClick(PresetMenuItem* MI)
	{
	    juce::Point<int> topRight;
        if(highlightQueue.isEmpty() || items.isEmpty())
        {
            topRight = vp.getScreenPosition().translated(getWidth(),0);
        }
        else
        {
              topRight = highlightQueue.getLast()->getScreenPosition().translated(highlightQueue.getLast()->getWidth(), 0);
        }
		//auto currentMI = highlightQueue.getLast();
		//auto topRight = highlightQueue.isEmpty() ? getScreenPosition().translated(getWidth(), 0) : currentMI->getScreenPosition().translated(currentMI->getWidth(), 0); //places popup either next to last selected item or top of the menu
		Popup->hide();

		createPopupMenu(topRight, !highlightQueue.isEmpty());
	}
	...
	void PresetMenu::createPopupMenu(juce::Point<int>& topRight, bool highlighted)
	{
		//		auto wRat = 1 / 1.5;
		//		auto hRat = 1 / 6.0;
		...
		if (highlightQueue.size() == 1)
		{
			...
			names.add("Override Preset");
			returnValues.add(PresetItemActions::Override);
		...
		Popup->setScaledBounds(topRight.getX(), topRight.getY() - itemHeightRatio * parentManager->browser.getWidth() / 2.5 * BDSP_POPUP_OUTLINE_RATIO, parentManager->browser.getWidth() / 2.5, 0);
		auto display = juce::Desktop::getInstance().getDisplays().getDisplayForRect(Popup->getBounds());
		Popup->setTopRightPosition(juce::jmin(Popup->getRight(), display->userArea.getRight()), juce::jmin(Popup->getBottom(), display->userArea.getBottom()) - Popup->getHeight());
		Popup->show();
