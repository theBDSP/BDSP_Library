    ...
    inline const char* line_joint_fragment_shader = //fragment shader
        R"(
            ...
            void main()
            {
                ...
                {
                    vec2 dist = u_viewport * (pos- lineCenter)/u_thickness; // normalized distance from this fragment to the line center
                    float d = sqrt(dist.x*dist.x+dist.y*dist.y);
                    if(d<=1) // fragment is within radius of cap/joint
                    {
